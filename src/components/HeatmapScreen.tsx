    ...
    return (
      <div className="flex flex-col gap-4 pb-4">
        <div className="flex flex-col items-center py-16 text-center px-6">
          <p className="font-black text-[#18231f] text-xl mb-2">
            No habits to map yet.
          </p>
          <p className="text-[13px] text-[#657a71] mb-6">
    ...
    <div className="flex flex-col gap-4 pb-4">
        ...
        <div className="mt-4 flex items-center justify-between">
          <div className="flex gap-4">
            ...
            <div className="text-center">
              ...
              <p className="text-[10px] text-[#6db898] mt-0.5">
                Current streak
              </p>
            </div>
            <div className="text-center">
      ...
      <section className="bg-white border border-black/[0.06] rounded-2xl p-5">
        ...
        <div className="flex flex-wrap gap-1.5 mb-5">
          {habits.map((habit) => (
            <button
              ...
              className={[
                "px-3 py-1.5 rounded-xl text-[12px] font-semibold transition-all duration-150 active:scale-[0.96]",
          ...
          <div className="min-w-max">
            ...
            <div className="flex mb-1 ml-8">
              {columns.map((column, index) => (
                <div
                  key={`label-${index}`}
                  className="w-4 mx-0.5 text-[9px] text-[#657a71] font-semibold truncate"
                >
            ...
            <div className="flex gap-0.5">
              ...
              <div className="flex flex-col justify-around w-7 shrink-0">
                {weekdayLabels.map((label) => (
                  <span
                    key={label}
                    className="text-[9px] text-[#657a71] text-right pr-1 leading-none h-4 flex items-center justify-end"
                  >
                    {label}
                  </span>
                ))}
                  ...
                  {column.days.map((day) => (
                    <div
                      ...
                      style={
                        day.done
                          ? ({
                              background: selectedHabit.accent,
                            } as CSSProperties)
                          : day.scheduled
                            ? ({
        ...
        <div className="flex items-center gap-2 mt-4">
          <span className="text-[10px] text-[rgba(255,253,248,0.5)]">Less</span>
          <div className="w-3.5 h-3.5 rounded-[3px] bg-[rgba(255,253,248,0.08)]" />
          <div
          ...
          />
          <span className="text-[10px] text-[rgba(255,253,248,0.5)]">More</span>
        </div>
      </section>
    </div>
  );
